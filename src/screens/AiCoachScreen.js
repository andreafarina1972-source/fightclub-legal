...
import {
  ...
  translatePeriodizationPhase, translateWeightCategory, translateReadinessState,
...
// ─────────────────────────────────────────────────────────
// CARD MOTORE DECISIONALE — Fase 2 (superficie utente, 13/08/2026)
// ─────────────────────────────────────────────────────────
// Si calcola all'apertura della schermata via useLoadDecision (vedi hook:
// legge trainingLoad/recovery/baseline/readiness/periodizzazione/check-in
// da sola), NON dopo la generazione di un piano — l'atleta deve poter
// sapere se allenarsi oggi senza spendere una chiamata AI. Nessun dato
// salute grezzo qui: solo il livello, l'headline (chiave i18n), il codice
// del reason principale (tradotto via loadDecision.reasons.<code>), il
// readiness (solo score + state, mai i components) e i flag trasversali.
const LEVEL_COLORS = {
  progress: "#37E293",
  maintain: "#2D9CDB",
  reduce:   "#FF9500",
...
function LoadDecisionCard() {
  ...
  if (loading) {
    return (
      <View style={[ldStyles.card, { borderColor: "rgba(255,255,255,0.08)" }]}>
        <Text style={ldStyles.loadingText}>{t("loadDecision.card.loading") || "Calcolo il carico di oggi…"}</Text>
      </View>
    );
  }
  ...
  if (decision.level === "learning") {
    ...
    return (
      <View style={[ldStyles.card, { borderColor: "rgba(255,255,255,0.1)" }]}>
        <Text style={ldStyles.headline}>{t(decision.headline) || "Sto ancora imparando i tuoi ritmi"}</Text>
        <Text style={ldStyles.reason}>
  ...
  return (
    <View style={[ldStyles.card, { borderColor: levelColor + "33" }]}>
      <View style={ldStyles.headerRow}>
        <Text style={[ldStyles.headline, { color: levelColor }]}>{t(decision.headline) || decision.level}</Text>
        {readiness && (
          <View style={ldStyles.readinessBadge}>
            <Text style={[ldStyles.readinessScore, { color: readiness.color }]}>{readiness.score}</Text>
            <Text style={ldStyles.readinessState}>{translateReadinessState(readiness.state)}</Text>
          </View>
        )}
      </View>

      {reasonText ? <Text style={ldStyles.reason}>{reasonText}</Text> : null}

      {showChange && (
        <Text style={[ldStyles.change, { color: levelColor }]}>
      ...
      {decision.level === "rest" && (
        <Text style={ldStyles.softNote}>{t("loadDecision.card.restNote") || "È un'indicazione, non un divieto."}</Text>
      )}
      ...
      {decision.flags?.includes("possible_illness") && (
        <Text style={ldStyles.flagNote}>{t("loadDecision.card.illnessNote") || "Se questi segnali persistono nei prossimi giorni, considera di sentire un medico."}</Text>
      )}
      ...
      {decision.flags?.includes("aggressive_weight_cut") && (
        <Text style={ldStyles.flagNote}>{t("loadDecision.card.weightCutNote") || "Rivedi la traiettoria del peso con un professionista: qui non trovi indicazioni su come accelerarla."}</Text>
      )}
    </View>
  );
}
