...
export default function WorkoutRunScreen({ route, navigation }) {
  ...
  const finishWorkout = () => {
    ...
    // Nessuna stima VO2max qui: un BPM istantaneo di fine allenamento (spesso
    // defaticamento/recupero, mai uno sforzo massimale di 30s) non è un input
    // valido per il gate della formula Uth — vedi BRIEF-vo2max-verifica.md,
    // Intervento A. Il VO2max mostrato altrove (Home, share card) viene
    // SOLO dal test dedicato di HomeScreen (`latestVo2` in HistoryContext).
    addSession({
      type: 'workout',
